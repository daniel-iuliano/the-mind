...
const API_PROXY = import.meta.env.DEV ? "https://corsproxy.io/?" : "/api/coinex";

const buildCoinexUrl = (path: string, params?: Record<string, string | number>) => {
  const url = new URL(`${COINEX_BASE_URL}${path}`);
  ...
  if (API_PROXY.startsWith("http")) {
    return `${API_PROXY}${encodeURIComponent(url.toString())}`;
  }

  const proxyUrl = new URL(API_PROXY, window.location.origin);
...
export const fetchTop30Markets = async (): Promise<string[]> => {
  const proxiedUrl = buildCoinexUrl("/market/ticker/all");
...
export const fetchTicker = async (symbol: string): Promise<{ last: number, vol: number, change: number } | null> => {
    const proxiedUrl = buildCoinexUrl("/market/ticker", { market: symbol });
...
export const fetchCandles = async (symbol: string, timeframe: Timeframe): Promise<OHLCV[]> => {
  ...
  const proxiedUrl = buildCoinexUrl("/market/kline", {
    market: symbol,
    type: timeframe,
    limit: 100,
  });
